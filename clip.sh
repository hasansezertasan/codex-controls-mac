...
# sent with `clip send` can be pasted straight into a Codex session on the box
# with Ctrl-V (codex reads the box pasteboard; the terminal can't carry image
# bytes through a normal paste).
#
...
clip - sync the clipboard between this Mac and the box over SSH (text or image).
  ...
  clip send   this Mac's clipboard -> the box   (images: Ctrl-V into codex to attach)
